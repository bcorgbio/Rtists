...
```{r,"logistic modeling graphs for each species",fig.cap="Megaceryle alcyon",cache=TRUE}
...
```{r,"arrival graphs for each species",fig.cap="Sayornis phoebe",cache=TRUE}
ep.pred <- ep%>%
  group_by(year)%>%
  summarize(
    pred=predict(nls(prop~SSlogis(j.day,Asym, xmid, scal)),newdata=data.frame(j.day=min(j.day):max(j.day))),#predict the logistic curve for each species
...
```{r,"arrival graphs for each species",fig.cap="Vireo philadelphicus",cache=TRUE}
...
```
Note -- there are fewer years than the other species for the Philadelphia Virelo. This is due to the fact that there were too few occurrence data points for the omitted years to base a logistic curve off of. This is prevalent in a few other species as well. 

```{r,"arrival graphs for each species",fig.cap="Catharus ustulatus",cache=TRUE}
st.pred <- st%>%
  group_by(year)%>%
  summarize(
    pred=predict(nls(prop~SSlogis(j.day,Asym, xmid, scal)),newdata=data.frame(j.day=min(j.day):max(j.day))),#predict the logistic curve for each species
...
```{r,"arrival graphs for each species",fig.cap="Setophaga caerulescens",cache=TRUE}
...
```{r,"arrival graphs for each species",fig.cap="Setophaga dominica",cache=TRUE}
...
```{r,"what day of the year .25 of the population arrives, varying by year, fig.cap="Megaceryle alcyon",cache=TRUE}
#Megaceryle alcyon - belted kingfisher
...
```
```{r,"what day of the year .25 of the population arrives, varying by year, fig.cap="Sayornis phoebe",cache=TRUE}
...
ep.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varying by year, fig.cap="Vireo philadelphicus",cache=TRUE}
#Vireo philadelphicus - Philadelphia Virelo 
...
pv.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varying by year, fig.cap="Catharus ustulatus",cache=TRUE}
...
st.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varying by year, fig.cap="Setophaga caerulescens",cache=TRUE}
#Setophaga caerulescens - Black-throated Blue Warbler 
...
btw.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varying by year, fig.cap="Setophaga dominica",cache=TRUE}
#Setophaga dominica - Yellow-throated  Warbler 
...
These results seem to insinuate that there is relative inconsistency among arrival date from year to year, for each species. We will conduct further analysis with weather data from the migration path to see if weather has an effect on these species' arrival dates on an annual basis. 
...
```
```{r,"combining weather data and species arrival data"}
#Megaceryle alcyon - belted kingfisher
...
```{r,"linear models,non-averaged weather data", r message = FALSE}
mea.lmer <- lmer(j.day~tmin*tmax*wvec+(1|name),mea.arr.weath,na.action = "na.fail")
anova.mea <- Anova(mea.lmer)
kable(anova.mea,label = "Belted Kingfisher")

ep.lmer <- lmer(j.day~tmin*tmax*wvec+(1|name),ep.arr.weath,na.action = "na.fail")
anova.ep <- Anova(ep.lmer)
kable(anova.ep,label = "Eastern Phoebe")

pv.lmer <- lmer(j.day~tmin*tmax*wvec+(1|name),pv.arr.weath,na.action = "na.fail")
anova.pv <- Anova(pv.lmer)
kable(anova.pv,label="Philadelphia Virelo")

st.lmer <- lmer(j.day~tmin*tmax*wvec+(1|name),st.arr.weath,na.action = "na.fail")
anova.st <- Anova(st.lmer)
kable(anova.st,label="Swainson's Thrush")

btw.lmer <- lmer(j.day~tmin*tmax*wvec+(1|name),btw.arr.weath,na.action = "na.fail")
anova.btw <- Anova(btw.lmer)
kable(anova.btw,label="Black Throated Blue Warbler")

yw.lmer <- lmer(j.day~tmin*tmax*wvec+(1|name),yw.arr.weath,na.action = "na.fail")
anova.yw <- Anova(yw.lmer)
kable(anova.yw,label="Yellow-Throated Warbler")
```

Based off of the values the ANOVA test produces, using only single-day weather data at each location doesn’t reveal any significant relationships between weather and arrival day. The variables that are being assessed in this model are maximum temperature of the area (tmax), minimum temperature of the area (tmin), and then a wind vector value, calculated using average wind velocity in meters per second and the direction of the fastest 2-minute wind.  

```{r,"linear models, mean-weather over 2 weeks", r message = FALSE}
mea.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),mea.arr.weath2,na.action = "na.fail")
anova.mea2 <- Anova(mea.lmer2)
kable(anova.mea2,label = "Belted Kingfisher")

ep.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),ep.arr.weath2,na.action = "na.fail")
anova.ep2 <- Anova(ep.lmer2)
kable(anova.ep2,label="Eastern Phoebe")

pv.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),pv.arr.weath2,na.action = "na.fail")
anova.pv2 <- Anova(pv.lmer2)
kable(anova.pv2,label="Philadelphia Virelo")

st.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),st.arr.weath2,na.action = "na.fail")
anova.st2 <- Anova(st.lmer2)
kable(anova.st2,label="Swainson's Thrush")

btw.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),btw.arr.weath2,na.action = "na.fail")
anova.btw2 <- Anova(btw.lmer2)
kable(anova.btw2,label="Black-Throated Blue Warbler")

yw.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),yw.arr.weath2,na.action = "na.fail")
anova.yw2 <- Anova(yw.lmer2)
kable(anova.yw2,label="Yellow-Throated Warbler")
```

In the second set of linear models, instead of using weather data from single-days in time, we calculated averages of the weather data variables over the span of 2 weeks, to account for any birds that don't all arrive / leave at the same time. With these weather means, there do appear to be significant fixed effects of weather, in many of the species -- all three of the variables seem to have an effect on the arrival date. 

# Secondary Model with Single-Day Weather data variables
```{r,"secondary models", r message = FALSE}
mea.arr.aic1 <- dredge(mea.lmer,fixed = c("tmin","tmax","wvec"),)
mea1.kb <- kable(mea.arr.aic1[1:4,],caption = "Fit values for nested models of the most complicated lme model for Belted Kingfisher")
print(mea1.kb)
ep.arr.aic1 <- dredge(ep.lmer,fixed = c("tmin","tmax","wvec"),)
ep1.kb <- kable(ep.arr.aic1[1:4,],caption = "Fit values for nested models of the most complicated lme model for Eastern Phoebe")
print(ep1.kb)

pv.arr.aic1 <- dredge(pv.lmer,fixed = c("tmin","tmax","wvec"),)
pv1.kb <- kable(pv.arr.aic1[1:4,],caption = "Fit values for nested models of the most complicated lme model for Philadelphia Virelo")
print(pv1.kb)

st.arr.aic1 <- dredge(st.lmer,fixed = c("tmin","tmax","wvec"),)
st1.kb <- kable(st.arr.aic1[1:4,],caption = "Fit values for nested models of the most complicated lme model for Swainson's Thrush")
print(st1.kb)

btw.arr.aic1 <- dredge(btw.lmer,fixed = c("tmin","tmax","wvec"),)
btw1.kb <- kable(btw.arr.aic1[1:4,],caption = "Fit values for nested models of the most complicated lme model for Black-Throated Blue Warbler")
print(btw1.kb)

yw.arr.aic1 <- dredge(yw.lmer,fixed = c("tmin","tmax","wvec"),)
yw1.kb <- kable(yw.arr.aic1[1:4,],caption = "Fit values for nested models of the most complicated lme model for Yellow-Throated Warbler")
print(yw1.kb)
```

We utilized a different model from the linear models above, getting rid of all but three weather variables (only kept tmin, tmax, and wvec). The data tables above indicate the best fitting models with the 3 weather variables as fixed effects. From these tables, we can surmise that the best fitting models, for each species, reflects all three variables have an effect on arrival date. 

#Secondary Model with Average weather data variables  
```{r,"secondary models",r message=FALSE}
mea.arr.aic <- dredge(mea.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),)
mea.kb <- kable(mea.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model for Belted Kingfisher")
print(mea.kb)

ep.arr.aic <- dredge(ep.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),)
ep.kb <- kable(ep.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model for Eastern Phoebe")
print(ep.kb)

pv.arr.aic <- dredge(pv.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),)
pv.kb <- kable(pv.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model for Philadelphia Virelo")
print(pv.kb)

st.arr.aic <- dredge(st.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),)
st.kb <- kable(st.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model for Swainson's Thrush")
print(st.kb)

btw.arr.aic <- dredge(btw.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),)
btw.kb <- kable(btw.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model for Black-Throated Blue Warbler")
print(btw.kb)

yw.arr.aic <- dredge(yw.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),)
yw.kb <- kable(yw.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model for Yellow-Throated Warbler")
print(yw.kb)
```
