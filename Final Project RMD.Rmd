...
```{r,"logistic modeling graphs for each species",fig.cap="Belted Kingfisher",cache=TRUE}
...
```{r,"log models",fig.cap="Eastern Phoebe",cache=TRUE}
ep<- dat%>%
...
```{r,"log models2",fig.cap="Philadelphia Virelo",cache=TRUE}
pv<- dat%>%
...
There are fewer years than the other species for the Philadelphia Virelo. This is due to the fact that there were too few occurrence data points for the omitted years to base a logistic curve off of. This is prevalent in a few other species as well. 

```{r,"log model 3",fig.cap="Swainson's Thrush",cache=TRUE}
st <-  dat%>%
...
```{r,"log model 4",fig.cap="Black-throated Blue Warbler",cache=TRUE}
...
```{r,"log model 5",fig.cap="Yellow-Throated Warbler",cache=TRUE}
...
```{r,"what day of the year .25 of the population arrives, varying by year", fig.cap="Belted Kingfisher",cache=TRUE}
#Megaceryle alcyon - belted kingfisher
...
```
```{r,"what day of the year .25 of the population arrives, varying 1 ", fig.cap="Eastern Phoebe",cache=TRUE}
...
ep.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varyin 2", fig.cap="Philadelphia Virelo",cache=TRUE}
#Vireo philadelphicus - Philadelphia Virelo 
...
pv.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varying by year3", fig.cap="Swainson's Thrush",cache=TRUE}
...
st.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varying by year4", fig.cap="Black-Throated Blue Warbler",cache=TRUE}
#Setophaga caerulescens - Black-throated Blue Warbler 
...
btw.arrive.date%>%
  ggplot(aes(year,j.day))+geom_point()
```
```{r,"what day of the year .25 of the population arrives, varying by year5", fig.cap="Yellow-Throated Warbler",cache=TRUE}
#Setophaga dominica - Yellow-throated  Warbler 
...
kable(anova.pv,caption="Philadelphia Virelo")
...
kable(anova.pv2,caption="Philadelphia Virelo")
...
kable(pv.arr.aic1[1:4,],caption = "Fit values for nested models of the most complicated lme model for Philadelphia Virelo")
...
kable(pv.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model for Philadelphia Virelo")
...
kable(bestpv1,caption="Philadelphia Virelo")
...
kable(bestpv,caption="Philadelphia Virelo")
...
These tables dispaly the ANOVA values for the best-fit models found using only the 3 weather variables as fixed effects. There appears to be some significance for these weather variables having a fixed effect on the arrival date of the species. 
